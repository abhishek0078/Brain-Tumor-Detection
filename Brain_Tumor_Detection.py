...
best_model = load_model(filepath='models/cnn-parameters-improvement-01-0.98.model')
...
def prediction_fn(image):
    class_name = ['No','Yes']
    img_batch = np.expand_dims(image, axis=0)
    prediction = model.predict(img_batch,batch_size=None)
    if prediction > 0.5:
        a = 1
    elif prediction < 0.5:
        a = 0
